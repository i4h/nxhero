...
module.exports = {
    ...
    render: function(menuActions, selections, data, callback) {
        ...
        module.exports.renderCurrent(currentAction, breadcrumbs.join("-"), data, function(err, selection) {

            /* If there was an error, finish */
            if (err !== null) {
                log.error("Unable to render action " + breadcrumbs.join("-"));
    ...
    renderCurrent:function(currentAction, breadcrumbs, data, callback) {
        ...
        inquirer.prompt(question).then(function(answers) {
            if (Array.isArray(question))
                ...
                callback(null, answers[question.name])
        }).catch(function(p) {
            log.error("Caught  Promise  Rejection: " + p);
            return callback(new Error("Could not resolve action"));
        });
    }
