...
module.exports = {
    ...
    handleElementFilterUpdate : function(models, filters, userInput, data) {
        if (models.lenght === 0) {
            log.info("Can not filter on empty element list");
            return;
        }
        ...
        switch(userInput[0]) {
            case "s" :
                ...
                if (userInput[1].length === 0) {
                    log.info("Ignoring empty selection.")
                    break;
                }
                ...
                for (var i = 0; i < ids.length; ++i) {
                    ...
                    if (typeof name === "undefined") {
                        /* Tag filter just gets selection[tagname ] = tagname */
                        filters[elementName].selection[id] = id;
                    } else
                        /* Other element filters get this */
    ...
    handleDirectFilterUpdate : function(filterName, filters, userInput, options) {
        switch(userInput[0]) {
            ...
            case "n":
                ...
                filters[filterName].name = userInput[1];
                break;
        }
    },
    ...
    getJobsFiltered : function(store, filters, options, callback) {
        ...
        Job = Job.join({tag: null, jobgroup: "binary", problem: null});
    ...
    getDataFromCmdLineArgs : function(store, options, callback) {
        var filterFile = nconf.get("file");
        var jobIds = nconf.get("jobids");
        var jobgroupIds= nconf.get("jobgroupids");
        ...
        if (typeof filterFile !== "undefined") {
            filters = JSON.parse(fs.readFileSync(filterFile, 'utf8'));
        } else {
            if (typeof jobIds !== "undefined") {
                filters.jobs = {selection: parseIds(jobIds)};
            }

            if (typeof jobgroupIds !== "undefined") {
                filters.jobgroups = {selection: parseIds(jobgroupIds)};
            }
        }
        ...
        module.exports.getJobsFiltered(store, filters, {joinValues: {parameters: true}}, function(jobs) {
            callback(null, {jobs: jobs, filters: filters});
        });
    },
