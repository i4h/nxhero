...
module.exports = {
    ...
    listHeader: [{content:'Name', colWidth: 40}, {content: 'Type', colWidth: 10}, {content: 'Binaries', colWidth:60, lineWrap: true, wordSep: ", "} ],
    ...
    getFloatParameterIds: function(store, options, callback) {
        var Model = store.Model("Parameter");
        var ids = [];
        Model.where({type: this.typeFloat}).exec(function(records) {
            for (var idx = 0; idx < records.length; idx++) {
                record = records[idx];
                ids.push(record.id);
            }
