...
module.exports = {
    ...
    getModelList: function(store, modelName, options, callback) {
        store.verify();
        var BaseModel = store.getBaseModel(modelName);
        var Model = store.Model(modelName);
        if (options.order)
        ...
        Model.where().join(joinConf).exec(function(records) {
            ...
            for (var idx = 0; idx < records.length; idx++) {
                record = records[idx];
                rows.push(record.getListRow());
            }
            callback(null, rows, BaseModel.listHeader);
        });
    },
