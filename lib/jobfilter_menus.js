...
module.exports = {
                ...
                menu: {
                    ...
                    t: JobFilter.getElementFilterMenu('(t)ag', 'Tag', function (input) {
                        return {
                            ...
                            name: 'ids',
                            message: 'Tags:',
                            choices: Object.keys(input.data.tagsUnique),
                        };
                    }),
                    p: {
                        label : '(p)arameter',
                    ...
                    s: JobFilter.getElementFilterMenu('(s)tatus', 'Status', function (input) {
                        return {
                            ...
                            choices: Object.keys(input.data.executionStatuses)
                        };
                    }),
                    e: JobFilter.getElementFilterMenu('r(e)t. status', 'Return Status', function (input) {
                        return {
                            ...
                            choices: Object.keys(input.data.returnStatuses)
                        };
                    }),
                    c: JobFilter.getElementFilterMenu('su(c)cess', 'Success', function (input) {
                        return {
                            ...
                            name: 'ids',
                            message: 'Success:',
                            choices: Object.keys(input.data.success)
                        };
                    }),
                    r: {
                        label: "(r)eset",
            ...
            Menu.render(module.exports.menuActions, [], {data: data, filters: filters}, function(err, userInput) {
                ...
                switch(first) {
                    ...
                    case 'p':
                        ...
                        return module.exports.process(store, filters, data, {}, userInput, function(err) {
                            if (err)
                                throw err;
    ...
    /** Gets a selection string for the first argument
     * There are three cases:
     * - objects is an array of objects: get list of names
     * - objects is an object: contains unique counts
     * - objects is an array of strings: contains unique column values
     *
     * @param objects
     * @param name
     * @param nSummary
    ...
    getSelectionString: function(objects, name, nSummary) {
        ...
        if (!Array.isArray(objects)) {
            ...
            string +=  pad(name + ":", module.exports.labelPad) + pad(module.exports.numberPad, String(totalCount));
            if (Object.keys(objects).length < nSummary) {
                let elements = [];
                if (Object.keys(objects).length === 1) { /* No count if only one value */
                    elements.push(Object.keys(objects)[0]);
                } else {
                    for (var value in objects)
                        elements.push(`${value} (${objects[value]})`);
                }
                string += " (" + elements.join(", ") + ")";
            }
        } else {
            string +=  pad(name + ":", module.exports.labelPad) + pad(module.exports.numberPad, String(objects.length));
            if (objects.length >= 1) {
    ...
    getFilteredViewData : function(store, data, filters, options, callback) {
        ...
        let filtered = (Object.keys(filters).length >= 1);
        JobFilter.getJobsFiltered(store, filters, {}, function(jobs) {
            if (filtered && jobs.length >= 1000 && nconf.get('database').type === "sqlite") {
                filtered = false;
                log.error("Filter includes more than 1000 jobs. Please refine to avoid hitting sqlite limits");
            }
            async.parallel([
                ...
                function(callback) {
                    module.exports.getUniqueParamsAndValues(store, filtered ? ArrayHelper.getColumnUnique(jobs, "id", {type: "int"}): null, {}, callback);
                },
                /* Get execution statuses */
                function(callback) {
                    return callback(null, ArrayHelper.getColumnUniqueCounts(jobs, "execution_status", {}));
                },
                /* Get return statuses */
                function(callback) {
                    return callback(null, ArrayHelper.getColumnUniqueCounts(jobs, "return_status", {}));
                },
                /* Get success columns */
                function(callback) {
                    return callback(null, ArrayHelper.getColumnUniqueCounts(jobs, "success", {}));
                },
                /* Get tags */
                function(callback) {
                    return BaseTag.getJobsTags(store, jobs, {}, callback);
                },
                ...
                data = {
                    ...
                    success: results[5],
                    tags: results[6],
                    tagsUnique: ArrayHelper.getColumnUniqueCounts(results[6], "name"),
                };
                return callback(null, data);
            });
        });
    ...
    getUniqueParamsAndValues : function(store, jobIds, options, callback) {
        var calls = [];
        var callClosure = function(valueModel) {
            return function(callback) {
                if (jobIds !== null)
                    valueModel = valueModel.where({job_id: jobIds});

                valueModel.join("parameter").exec(function(records) {
     ...
     * @param callback
     */
    process : function(store, filters, data, options, what, callback) {
        var processorId = what[0];
        var processor = BaseProcessor.registry[processorId];

        /* Get jobs again with parameters and parameter values */
        JobFilter.getJobsFiltered(store, filters, {joinValues: {parameters: true}}, function(jobs) {
            data.jobs = jobs;
