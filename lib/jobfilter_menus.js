...
module.exports = {
                    ...
                    p: {
                        ...
                        question : function (input) {
                            ...
                            questions.push({
                                ...
                                name: "param",
                                choices: input.data.parameters.parameters
                            });
                            questions.push({
                                type: 'checkbox',
                                ...
                                choices: function(answers) {
                                    return input.data.parameters.valuesByName[answers.param];
                                }
                            })
                            return questions;
                        }
    ...
    listParameters : function(parameters, options) {
        ...
        var nameColWidth = 0;
        for (var i in parameters.valuesByName)
            nameColWidth = Math.max(nameColWidth, i.length);

        for (var i in parameters.valuesByName) {
            var name = i;
            var values = parameters.valuesByName[i];
            log.verbose(pad(i + ":", nameColWidth + 2) + values.join(", "));
        }
        ...
        async.parallel(calls, function(err, results) {
            ...
            for (var i in BaseParameter.valueModels) {
                ...
                for (var j = 0; j < results[counter].length; ++j) {
                    ...
                    paramsById[paramVal.parameter.id] = paramVal.parameter.name;
